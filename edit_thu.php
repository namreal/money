...
<head>
    ...
    <!-- Title Page-->
    <title>Thay đổi thông tin các khoản thu</title>
...
<body>
    ...
    <form method="POST">
        <?php
            ...
            $id=$_GET['id'];
            $query=$DB->query("select * from thu where id='$id'");
            $row=mysqli_fetch_assoc($query);
        ?>
        <div class="div">
            <div id="padding"><a href="index.php"><span class="close">&times;</span></a></div>
            <div>
                <div class="flexbox">
                    <div display="flex">
                        <div style="flex: 1;">
                ...
                <div class="flex">
                    <button class="btn" name="update_thu" type="submit"><i class="fa fa-plus"></i> Cập nhật</button>
                </div>
            </div>
        </div>
        <?php
            if (isset($_POST['update_thu'])){
                $id=$_GET['id'];
                $date =$_POST['date'];
                $money=$_POST['money'];
                $note=$_POST['note'];
                        
                $sql = "UPDATE thu SET `date`='$date', `money`='$money', `note`='$note' WHERE id='$id'";
